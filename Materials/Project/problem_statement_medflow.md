# Problem Statement: "MedFlow" Clinical Equipment Command Center
...
Clinical operations leadership cannot easily answer crucial operational questions, making equipment utilization and maintenance planning difficult. As part of a clinical operations modernization initiative, Halcyon Health Systems needs a **centralized, full-stack Command Center** to track equipment inventories, manage work order assignments, upload service/diagnostic reports, and monitor real-time equipment health across all hospital and clinic sites.

---

## 2. Key Business Questions

The new system must allow clinical operations admins and biomed technicians to easily answer the following analytical questions:

* **Low Charge Alert:** *Which active devices are operating below a 20% charge level across all facilities?*
* **Co-Location Discrepancy:** *How many devices are assigned to biomed technicians who are NOT co-located at the same physical hospital?*
* **Reliability Metrics:** *What is the work order completion/failure ratio broken down by device model?*
* **Maintenance Flags:** *Which hospitals have more than 30% of their equipment currently flagged for maintenance?*
* **Reporting Lines:** *How many technicians reporting to a specific Regional Biomed Supervisor have active work orders assigned to them?*
...
1. **Hospitals:** Physical sites housing equipment pools (`id`, `name`, `location_region`, `capacity`, `supervisor_id`).
2. **Equipment:** Individual medical devices (`id`, `serial_number`, `model`, `status`: *Available* | *In-Use* | *Maintenance* | *Offline*, `charge_level`, `facility_id`).
3. **Work Orders:** Service/deployment tasks assigned to equipment (`id`, `title`, `priority`: *Low* | *Medium* | *Critical*, `status`: *Pending* | *In-Progress* | *Completed* | *Failed*, `equipment_id`, `technician_id`).
4. **Service Reports:** Maintenance attachments and inspection files (`id`, `work_order_id`, `file_url`, `notes`, `created_at`).
...
* **Clinical Admin:** Full CRUD permissions across hospitals, equipment, work orders, and user accounts.
* **Field Technician:** Can view assigned equipment, trigger work order status changes, and attach service reports.
...
* Utilize **MUI DataGrid** to display equipment and work order listings with support for live sorting, search filtering, and pagination.
...
│   └── seed.sh         # Script to seed PostgreSQL with mock hospitals, equipment & users
